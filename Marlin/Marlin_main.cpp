 ...
 *
 * M101 - extend plate loader
 * M102 - retract plate loader
 *
 * "T" Codes
 *
...
#include <HCSR04.h>   //added for multiprinter
...
// following constants and variables added for multiprinter /////////////////////////////////// n 

///// Multiprinter Constants /////

int servoPositions[] = {15,150};

HCSR04 hc(58,57);
//Servo multiprinterPlatePusherServo;

//pin 11 is pwm safe
//pin 3 is not? pwm safe
//pin 4 is pwm safe
//pins 5, 6, and 2 do not seem to be pwm safe

//Pins Start
//const int multiprinterPlatePusherServoPin = 3;
const int loaderMotorPinMinus = 5;    
const int loaderMotorPinPlus = 4;     
const int intakeMotorPinPlus = 6;    
const int intakeMotorPinMinus = 11; 
   
const int xPrimeEndstopPin = 14;    //58;
const int servoExtendPin = 40;
const int servoRetractPin = 42;
const int switchOnePin = 3;
const int switchTwoPin = 2;
const int switchThreePin = 63;
//Pins End

///// Multiprinter Variables /////

boolean xPrimeEndstopState = false;
boolean switchOneState = false;   //returns true when pressed and false when un-pressed
boolean switchTwoState = false;
boolean switchThreeState = false;
int loaderMotorCurrent = 0;

int loaderMotorCurrentThreshold = 0;
int loaderMotorState = 0;
int intakeMotorState = 0;

int stateIndex = 0;
long int stateTimeMS = 0;
int stateTimeMSThreshold = 2000;

long int startTime = 0;

//boolean first = false;

///// End Multiprinter Variables /////
...
//#include "servo.h"
#if HAS_SERVOS  //multiprinter edit
  #include "servo.h"
#endif
...
//Servo multiprinterPlatePusherServo;   //multiprinter edit
#if HAS_SERVOS
...
//begin multiprinter functions

void multiprinterUpdateSensors(){
    if((millis() % 10) == 0 || true){
      xPrimeEndstopState = !digitalRead(xPrimeEndstopPin);
...
void multiprinterLoaderMotorPower(int power){   //takes in an interger from -100 to +100
    if(power > 0){
        analogWrite(loaderMotorPinPlus, power*255/100);
        analogWrite(loaderMotorPinMinus, 0);
        //MOVE_SERVO(1, power*180/100);
        //MOVE_SERVO(2, 0);
        
      }
    else{
        analogWrite(loaderMotorPinPlus, 0);
        analogWrite(loaderMotorPinMinus, power*255/100);
        //MOVE_SERVO(1, 0);
        //MOVE_SERVO(2, power*180/100);
      }  
  }

void multiprinterIntakeMotorPower(int power){   //takes in an interger from 0-100
    if(power > 0){
        analogWrite(intakeMotorPinPlus, power*255/100);
        analogWrite(intakeMotorPinMinus, 0);
      }
    else{
        analogWrite(intakeMotorPinPlus, 0);
        digitalWrite(intakeMotorPinMinus, power*255/100);
      }  
  }

void multiprinterServoExtend(){   
   digitalWrite(servoExtendPin, HIGH);
   digitalWrite(servoRetractPin, LOW);
  }

void multiprinterServoRetract(){   
   digitalWrite(servoExtendPin, LOW);
   digitalWrite(servoRetractPin, HIGH);
  }

void multiprinterServoOff(){   
   digitalWrite(servoExtendPin, LOW);
   digitalWrite(servoRetractPin, LOW);
  }

void multiprinterIntakeStop(){
    multiprinterIntakeMotorPower(0);
  }

void multiprinterLoaderStop(){
    multiprinterLoaderMotorPower(0);
  }

float multiprinterCalculateZShelf(){
    //return 50.0; //added for troubleshooting make sure to comment out later
    multiprinterUpdateSensors();
    int printHeight = current_position[2]; //use marlin variable here
    int previousShelfHeight = -10*hc.dist()+293;
    int zHeight = (((previousShelfHeight+printHeight + 5) - 82)/28)*28 + 82 + 28+2;
    if(zHeight <0 || zHeight > 262){
        lcd_setstatus("error invalid shelf height");
        return 260.0;
      }
    lcd_setstatus(zHeight);
    return float(zHeight)+0.5;
    //Serial.println(zHeight);
  }

void multiprinterIntakeSpit(){
    multiprinterUpdateSensors();
    multiprinterIntakeMotorPower(100);
    startTime = millis();
    while(millis()-startTime < 5000){
        multiprinterUpdateSensors();
        idle();
      }
    /*while(switchTwoState){ //millis()-startTime < 3000
        multiprinterUpdateSensors();
        idle();
      }*/
    multiprinterIntakeStop();
  }

/*void multiprinterIntakeIntake(){
    multiprinterUpdateSensors();
    //multiprinterIntakeMotorPower(-15);  //go back to atleast -40 later
    while(!switchThreeState){
        multiprinterUpdateSensors();
        idle();
      }
    multiprinterIntakeStop();
...
void multiprinterLoaderRetract(){
    startTime = millis();
    //Serial.println("retract start");
    while(millis()-startTime < 100){
        multiprinterLoaderMotorPower(-50);   //turn on the motor to push the plate forwards -40 is a good value
        idle();                 //run vital printer processes
        //Serial.println("time hasn't elasped yet");
      }
    multiprinterUpdateSensors();
    while(!xPrimeEndstopState){
        multiprinterUpdateSensors();
        idle();                 //run vital printer processes
        //Serial.println("endstop not pressed yet");
      }
    multiprinterLoaderMotorPower(0);        //turn off the motor once it's reached the desired position
    //Serial.println("move successful");
  }
  
void multiprinterLoaderExtend(){
    startTime = millis();
    //Serial.println("extend start");
    while(millis()-startTime < 100){
        multiprinterLoaderMotorPower(60);   //turn on the motor to push the plate forwards 40 is a good value //changed to 60 from 50 after the motor stalled in the last 10mm of motion
        idle();                 //run vital printer processes
        //Serial.println("time hasn't elasped yet");
        //lcd_setstatus("time hasn't elasped yet");
      }
    multiprinterUpdateSensors();
    if(xPrimeEndstopState){
        //lcd_setstatus("already pressed");
      }
    while(!xPrimeEndstopState){
        multiprinterUpdateSensors();
        idle();                 //run vital printer processes
        //Serial.println("endstop not pressed yet");
        lcd_setstatus("endstop not pressed yet");
      }
    multiprinterLoaderMotorPower(0);        //turn off the motor once it's reached the desired position
    //Serial.println("move successful");
    //lcd_setstatus("move successful");
  }

void multiprinterLoadPlate(){
  startTime = millis();
  multiprinterUpdateSensors();
  multiprinterIntakeMotorPower(-100);
  while(!switchThreeState || !xPrimeEndstopState){
      idle();
      multiprinterUpdateSensors();
      if(millis()-startTime < 200){
          multiprinterLoaderMotorPower(45);
        }
      if(millis()-startTime >= 100 && xPrimeEndstopState){
          multiprinterLoaderMotorPower(0);
        }
      if(switchThreeState){
          multiprinterIntakeStop();
...
void gcode_M101(){    //extends the plateloader and intakes the plate, checking for faults
    multiprinterServoRetract();
    lcd_setstatus("running M101");
    multiprinterLoaderMotorPower(0);
    multiprinterIntakeMotorPower(0);
//    multiprinterPlatePusherServo.move(servoPositions[0]);
    //MOVE_SERVO(0, servoPositions[0]);
    //servo[0].move(15);
    multiprinterServoRetract();
    multiprinterLoaderRetract();
    int unloadHeight = multiprinterCalculateZShelf();
    multiprinterServoRetract();
    do_blocking_move_to_z(unloadHeight, 25.0);          //move z to correct height to spit the plate
    //do_blocking_move_to_z(225.0, 25.0);       // hardcode as 225mm to start
    multiprinterIntakeSpit();
    multiprinterServoRetract();
    do_blocking_move_to_z((unloadHeight-31), 25.0);
    multiprinterServoExtend();
//    multiprinterPlatePusherServo.move(servoPositions[1]);
//    multiprinterPlatePusherServo.move(servoPositions[0]);
    //servo[0].attach(SERVO0_PIN);
    delay(2000);
    multiprinterServoRetract();
    delay(1000);
    //multiprinterServoOff();
    //MOVE_SERVO(0, servoPositions[1]);
    //MOVE_SERVO(0, servoPositions[0]);
    //servo[0].detach(); // Just set up the pin. We don't have a position yet. Don't move to a random position.
    do_blocking_move_to_z(0.2, 25.0);           //move to z = 0.5ish mm
    multiprinterUpdateSensors();
    if(switchOneState){
      //delay(1000);
      multiprinterLoadPlate();
    }
    else{
        multiprinterLoaderExtend();
      }
    multiprinterLoaderMotorPower(0);
    multiprinterIntakeMotorPower(0);
  }

void gcode_M102(){    //extends the plateloader and intakes the plate, checking for faults
    multiprinterUpdateSensors();
    if(!switchThreeState){
      lcd_setstatus("running M102");
      while(!switchOneState){
          multiprinterUpdateSensors();
          idle();
          lcd_setstatus("Waiting for plate in hopper");
          //add an lcd button press to exit manually
          if(switchOneState){
              delay(1000);
            }
        }
      ...
      multiprinterIntakeMotorPower(0);
      multiprinterLoaderRetract();
      //do_blocking_move_to_z(multiprinterCalculateZShelf(), 25.0);          //move z to correct height to spit the plate
      //do_blocking_move_to_z(225.0, 25.0);       // hardcode as 225mm to start
      //multiprinterIntakeSpit();
      do_blocking_move_to_z(0.2, 25.0);           //move to z = 0.5ish mm
      //delay(1000);
...
void servo_init() {
    ...
    servo[0].attach(SERVO0_PIN,1000,2000);
    servo[0].detach(); // Just set up the pin. We don't have a position yet. Don't move to a random position.
  #endif
    ...
    servo[3].detach();
  #endif
  #if NUM_SERVOS >= 5 && HAS_SERVO_4    //added for multiprinter
...
void process_parsed_command() {
  ...
  switch (parser.command_letter) {
    ...
    case 'M': switch (parser.codenum) {
      ...
      //multiprinter cases:
      case 101: gcode_M101(); break;                              //unload old plate and immediately after load new plate
      case 102: gcode_M102(); break;                              
      //end multiprinter cases
...
void setup() {
  ...
  //multiprinter setup
  pinMode(xPrimeEndstopPin, INPUT_PULLUP);
  pinMode(switchThreePin, INPUT_PULLUP);
  pinMode(switchOnePin, INPUT_PULLUP);
  //multiprinterPlatePusherServo.attach(multiprinterPlatePusherServoPin);
  pinMode(loaderMotorPinPlus, OUTPUT);
  pinMode(loaderMotorPinMinus, OUTPUT);
  pinMode(intakeMotorPinPlus, OUTPUT);
  pinMode(intakeMotorPinMinus, OUTPUT);
  pinMode(servoExtendPin, OUTPUT);
  pinMode(servoRetractPin, OUTPUT);
}
